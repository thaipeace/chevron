...
@Component({
  selector: 'app-connection-historical',
  templateUrl: './connection-historical.component.html',
  styleUrls: ['./connection-historical.component.scss']
})
export class ConnectionHistoricalComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.setHistoricConnections();
  }

  setHistoricConnections() {
    if (!this.startEndDate.length) {
      this.startEndDate = this.setStartEndDate([new Date(), new Date()]);
    }

    let getConnectionsPayload = new Payload(PayloadsConstant.activityManagement.getServiceConnectionHistory,
    ...
    this.apiDataService.executeQuery(getConnectionsPayload).subscribe(res => {
      ...
      this.historicConnections = (raw.APIResponse.Status === 'Success')
        ? this.dataUtilService.wrapObjToOneElementArray(raw.APIResponse.ExternalServiceConnectionHistories.ConnectionHistory)
        : [];

      this.tableData = new MatTableDataSource(this.historicConnections);
  ...
  onExport() {
    let exePayload = new Payload(PayloadsConstant.activityManagement.exportServiceconnectionhistorydata,
      [this.selectedApp.IntigratedServicename, this.selectedApp.ServiceFeature, this.startEndDate[0], this.startEndDate[1]]);
    this.apiDataService.executeQuery(exePayload).subscribe(res => {
      let raw = this.dataUtilService.convertXmlToJson(res);
      if (raw.DownloadLink) {     
        UtilsService.openNewWindow(raw.DownloadLink.ServiceConnectionHistoryData);
      } else {
