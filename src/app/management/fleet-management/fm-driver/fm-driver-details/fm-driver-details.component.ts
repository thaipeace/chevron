...
@Component({
    selector: 'app-fm-driver-details',
    templateUrl: './fm-driver-details.component.html',
    styleUrls: ['./fm-driver-details.component.scss']
})
export class FmDriverDetailsComponent implements OnChanges {
    @Input() id;
    @Input() index = 0;
    @Input() readonly = true;
        ...
        if (this.driverForm.valid) {
            ...
            this._DriverDataService.update(this.id, formData, this._authenService.getUsername()).then(rs => {
                if (rs && rs[1] && rs[1]['Update']['Status'] === 'Success') {
                    ...
                    this._DialogService.emitOutput(null, 'FmDriverDetailsDialogComponent');
                } else {
